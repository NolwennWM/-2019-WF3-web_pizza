...
function getOrderByUser($user) 
{
    ...
    $sql = "SELECT id, amount FROM orders WHERE id_user=:user || sess_user=:user ";
...
function updateOrderUser($order, $user)
{
    ...
    $sql = "UPDATE `orders` SET `id_user`=:id_user WHERE `id`=:id ";
...
function updateProductInOrder($product, $order_product_id)
{
    ...
    $sql = "UPDATE `order_products` SET `qty`=qty+1, `amount`=amount+".$product['price']." WHERE `id`=:id ";
